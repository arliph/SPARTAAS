validation <- function(D1,D2,valid){
  ...
  if(class(D2) != "dist"){
    #matrix ?
    if(!is.matrix(D2)){
      if(!is.data.frame(D2)){return("network data must be a data.frame.")}
      #number of col  ?
      if(valid){
        if(length(D2[1,])!=2){
          return(
            "Temporal network must have 2 colunms. Change data type parameter or try to inverse cont and network."
            )}
      }else{
        if(length(D2[1,])!=3){
          return("Time range must have 3 colunms. Change data type parameter or try to inverse cont and network.")}
      }
      if(valid){
        D2 <- adjacency(D2)
      }else{
  ...
  if(class(D1) != "dist") {
    #matrix ?
    if(min(D1) < 0){
      return("cont is a contingency table and can not contain negative value.")
    }
    if(!is.matrix(D1)){
  ...
  if(!length(D1) == length(D2)){return("cont and network have not the same size.")}
...
server <- function(input, output, session) {
    ...
    modalDialog(title="Sort Periods:",size=c("m"),
                ...
                column(4,
                  orderInput('hiatus', span(icon("plus",lib = "font-awesome"),'Add'), items = c("Hiatus"),
                             connect = 'seq2', as_source = TRUE, item_class = 'warning')
                ),
                column(8,
  ...
  import2Modal <- function(failed = FALSE) {
    modalDialog(title="Step 2:",size=c("l"),
                ...
                switchInput(inputId = "datatype",value = TRUE,
                            label = "<  Type  >",
                  onLabel = "Time range",
                  offLabel = "Network",
                  offStatus = "primary"
  ...
  output$timerange <- renderPlotly({
    ...
    res <- timerange.clust(data = data,cluster = values[["cluster"]])
    values[["order"]] <- res$order
    res$plot
  })

  output$prop_plot <- renderPlotly({
    dataset <- as.data.frame(values[["data1"]])
    dataset <- dataset[sort(labels(dataset)[[1]]),]

    niv1 <- c()
    for(i in 1:length(colnames(dataset))){
      niv1 <- c(niv1,rep(colnames(dataset)[i],length(rownames(dataset))))
    }
    data <- data.frame(
      niv1 = niv1,
      niv2 = niv1,
      nb = as.vector(unlist(dataset))
    )

    cluster <- as.vector(rep(values[["cluster"]],length(colnames(dataset))))
    order <- values[["order"]]$Cluster
    if(is.null(order)){
      order <- as.character(sort(unique(values[["cluster"]])))
    }else{
      if(max(nchar(as.vector(order))) > 1){
        order <- levels(order)
      }
    }


    res <- histogram.clust(data,cluster,order = order)
    res$plot
  })
  ...
  output$selectAlpha <- renderPlot({
    if(values[["firstClustSett"]]){
      ...
      sa <- perio.selectAlpha(D1, D2, method = values[["method"]],resampling=FALSE, ite= 5)
  ...
  output$dendrogramme <- renderPlot({
    ...
    mtext("Period:",side = 1, line = 1.5, at = 0, col = "#767676", cex=1.2)
    ...
    if(values[["ADD"]]){
      ...
      for(i in 1:length(unique(values[["clusterserio"]]))){
        if(!nchar(sort(unique(values[["clusterserio"]]))[i]) == 5){
          ...
          #y <- which(unique(values[["clusterserio"]][tree$order]) == sort(unique(values[["clusterserio"]]))[i])
          j <- trunc(sort(unique(values[["cluster"]]))[i])
          mtext("Rm", side = 1, line = 1.5, at = x, col = values[["truecolor"]][j], cex=1.5)
        }
      }
    }else{
      for(i in 1:length(unique(values[["clusterserio"]]))){
  ...
  output$CA <- renderScatterD3({
    ...
    res <- explor::prepare_results(CA(values[["data1"]]))
    xax = 1
    yax = 2
    var_sup = TRUE
    var_hide = "None"
  ...
  output$contentsD2 <- renderTable({
    ...
    if(input$datatype){values[["timerange"]] <- df}
    values[["D2"]] <- df
    return(head(df))
  },rownames=TRUE)
  ...
  output$spartaas <- renderImage({
    return(list(
      src = "www/SPARTAAS.jpg",
      width="900px",
  ...
  output$detail <- renderPlotly({
    ...
    if(class(D1)[1] == "perioclust_cl"){
      if(is.null(D1$D1) || is.null(D1$D2)){
        stop("We don't find dissmilarities in perioclust_cl object.")
      }else{
        D2 <- D1$D2
        D1 <- D1$D1
      }
        ...
        for(j in 1:nb_elt){
          if(j != i ){
            ...
            for(alpha in alpha_seq){
              ...
              tree <- hclust(mixt.dist,method=method)
