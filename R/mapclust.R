...
mapclust <-
  function (coord, var, label = "Nolabel", iter = 20, Plabel = TRUE, lonlat = TRUE)
  {
    #===================================================================================
    ...
    if(iter >= 1000){stop("The number of iterations is too high.")}
    ...
    if (rstudioapi::hasFun("showQuestion")) {
      if(!multi){
        ctrl <- rstudioapi::showQuestion("Data", "This classification requires geographical coordinates and a third variable: var.
                         \n We use var to weight the distance and var must be a frequency or density and always positive.
                                         \nThe values of var are:", ok = "Positive variable like density", cancel = "Other needed to be change in probability")
      }else{
        ctrl <- FALSE
      }
          } else {
      if(!interactive()){
        ...
        if(!multi){
          message("This classification requires geographical coordinates and a third variable: var.
                         \nWe use var to weight the distance and var must be a frequency or density and always positive.\n")
          ctrl <- readline("The values of var are like density and univariate ? TRUE or FALSE >> ")
          ctrl <- as.logical(ctrl)
          while(is.na(ctrl)){
            ctrl <- readline("The values of var are like density and univariate ? TRUE or FALSE >> ")
            ctrl <- as.logical(ctrl)
          }
    ...
    if(!ctrl){
      ...
      if(dim(as.data.frame(var))[2] > 1){
        if (rstudioapi::hasFun("showQuestion")) {
          showDialog("warning","var is multidimensionnal. You must transform it! var variables change forced!",url="")
        }else{
        ...
        if(min(var)<0){
          if (rstudioapi::hasFun("showQuestion")) {
            showDialog("warning","There are negatives values in var. The data are not like a count, probability or density.
                       var change forced.",url="")
          }else{
    ...
    plot1 <- plot1 + labs(title = "Hierarchical Clustering" , subtitle = "Spatialpatches")

    plot1 <- plot1 + theme(
      plot.margin = margin(0.5, 1, 0.5, 1, "cm")
    )
    res$dendrogram <- plot1
    ...
    plot <- plot + labs(title = "Hierarchical Clustering", subtitle = "Spatialpatches")
    #Axes x
...
"mapclust_cut_tree" <- function(classification, nb_grp = NA, dlim = NA)
{
  ...
  if(length(classification$dendrogram$theme)==2){
    Plabel <- TRUE
  }else{
    Plabel <- FALSE
  }
  ...
  print(m)
    ...
    out <- structure(list(X=classification$X,
                        ...
                        cutdlim=cutdlim,
                        lonlat=lonlat,
                        call=match.call()),class=c("ClustCutMap_cl","list"))
    cat(". Done")
    cat("\n\n\n
      Divisive hierarchical clustering
